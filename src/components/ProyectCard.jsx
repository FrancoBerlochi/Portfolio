...
const ProyectCard = ({ id, name, description, img, tech, links }) => {
  return (
    <div className="flex flex-col gap-4 p-6 min-[1750px]:max-[1921px]:h-[78vh] h-[82vh] bg-[#1b2647] rounded-2xl border border-transparent  transition-all duration-300 hover:border-[#08d5d4] hover:scale-[1.04] mb-6">
      <div className="min-[1750px]:max-[1921px]:w-85 min-[1750px]:max-[1921px]:h-85 h-75 max-md:w-full max-md:h-[40%]  w-70 bg-red-200">
        <img src={img[0]} alt="" className="object-cover w-full h-full" />
      </div>
      <h2 className="text-2xl max-md:text-[19px]">{name}</h2>
      <p className="text-xl text-gray-400 mb-4 max-md:text-[17px]">
        {description}
      </p>
      ...
      <div className="flex flex-col gap-2">
        <Link
          className="w-full bg-[#18d2d3] text-center text-white rounded-md py-2 max-md:py-1.5"
          to={`/${id}`}
        >
          Ver detalles
        </Link>
        ...
        <div className="flex justify-between items-center mx-2 gap-4">
          {links.length != 1 && (
            <a
              href={links[0]}
              target="_blanck"
              className="w-full py-2 text-center bg-[#173854] text-[#08d5d4] rounded-md max-md:py-1.5"
            >
              Ver Demo
            </a>
          )}
          <a
            ...
            target="_blanck"
            className="w-full py-2 text-center border-1 border-[hsl(180,43%,33%)] text-[#08d5d4] rounded-md max-md:py-1.5"
          >
            Github
          </a>
        </div>
